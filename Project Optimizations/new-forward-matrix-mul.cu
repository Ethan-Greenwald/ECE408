...
__global__ void mat_mul_conv(const float *mask, const float *input, float *output, const int Channel, const int K, const int Map_out, const int Width, const int Height) {
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    mat_mul_conv<<<grid, block>>>(device_mask, device_input, device_output, Channel, K, Map_out, Width, Height);
}
