...
__global__ void total(float *input, float *output, int len) {
  ...
  /* Each thread loads in 2 values, checking for out of bounds */
  partialSum[t] = (t+start) < len ? input[t+start] : 0;
  partialSum[blockDim.x + t] = (start + t + blockDim.x) < len ? input[start + t + blockDim.x] : 0;

  for(unsigned int stride = blockDim.x; stride >= 1; stride /= 2){
    __syncthreads();                          //ensure previous reduction step has finished
    if(t < stride)                            //if thread is part of this reduction step...
      partialSum[t] += partialSum[t+stride];  //...then perform a reduction
  }
