...
/*
no FP16:
Layer Time: 579.771 ms
Op Time: 12.6328 ms
Conv-GPU==
Layer Time: 447.717 ms
Op Time: 32.806 ms

Op Time: 12.7104 ms
Conv-GPU==
Layer Time: 459.357 ms
Op Time: 32.6274 ms
*/
...
__global__ void conv_forward_kernel(float* output, const float* __restrict__ input, const float* __restrict__ mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    #define tree(i2, i1, i0) TREE[i2 * TILE_WIDTH * Channel + i1 * Channel + i0]
    ...
    if(h < Height_out && w < Width_out){
        float acc = 0.0;
        const float* in_start = input + batch * (Channel * Height * Width) + c_in * (Height * Width);
        const float* mask_start = MASK + map * (Channel * K * K) + c_in * (K * K);
        ...
        for(int stride = 1; stride < Channel; stride *= 2){
            if(stride != 1)
                __syncthreads();
            if((c_in % (2*stride)) == 0 && (c_in + stride < Channel))
                tree(ty, tx, c_in) += tree(ty, tx, c_in + stride);
        }
        __syncthreads();
        if(c_in == 0)
            out_4d(batch, map, h, w) = tree(ty, tx, 0);
    }
...
__global__ void mat_mul_conv(float* output, const float* __restrict__ input, const float* __restrict__ mask, const int Map_out, const int Channel, const int Height, const int Width, const int K) {
    ...
    int numTiles = ceil((float)numACol/TILE_WIDTH);
        for (int i = 0; i < numTiles; i++) {
            ...
            if (load_col < numACol && row < Map_out) 
                maskTile[ty][tx] = __float2half(mask_4d(row, c, h, w));////////
            else 
                maskTile[ty][tx] = __float2half(0.0);
            ...
            if (load_row < numACol && col < Height_out*Width_out) 
                inputTile[ty][tx] = __float2half(in_4d(batch, c, h+p, w+q));/////////////
            else 
                inputTile[ty][tx] = __float2half(0.0);
            __syncthreads();
            ...
            */
            if((row < Map_out) && (col < Width_out*Height_out)){
                ...
                for(int i = 0; i < TILE_WIDTH; i++)
                    result = __hfma(maskTile[ty][i], inputTile[i][tx], result);
            }
            __syncthreads();
        }
    ...
    if ((row < Map_out) && (col < Width_out*Height_out))
        out_4d(batch, row, col / Width_out, col % Width_out) = __half2float(result);//////////
    #undef out_4d
    #undef in_4d
    #undef mask_4d
}
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    // if(isFirstLayer)
        conv_forward_kernel<<<grid, block, sizeof(float) * TILE_WIDTH * TILE_WIDTH * Channel>>>(device_output, device_input, device_mask, Batch, Map_out, Channel, Height, Width, K);
    // else
