...
/*
before reduction: #15 @ 44.2209 ms
after reduction: #14 @ 41.7008 ms

Layer Time: 7.06388 ms
Op Time: 0.12423 ms
Conv-GPU==
Layer Time: 5.59555 ms
Op Time: 0.346584 ms

Layer Time: 6.80784 ms
Op Time: 0.122098 ms
Conv-GPU==
Layer Time: 5.80767 ms
Op Time: 0.347128 ms
*/
...
__global__ void conv_forward_kernel(float* output, const float* __restrict__ input, const float* __restrict__ mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    if(h < Height_out && w < Width_out){
        ...
        for(int p = 0; p < K; p++){
            ...
            #pragma unroll 7
            for(int q = 0; q < K; q++)
                acc += tile_3d(tz, ty + p, tx + q) * mask_4d(map, tz, p, q);
        }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    if(isFirstLayer)
        conv_forward_kernel<<<grid, reductionBlock, sizeof(float) * Channel * (TILE_WIDTH + K - 1) * (TILE_WIDTH + K - 1)>>>(device_output, device_input, NULL, Batch, Map_out, Channel, Height, Width, K);
    else
