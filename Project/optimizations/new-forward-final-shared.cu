...
/*
before shared: #14 @ 45.1963 ms

16:
Layer Time: 7.01204 ms
Op Time: 0.127991 ms
Conv-GPU==
Layer Time: 5.77441 ms
Op Time: 0.401386 ms
        ...
        for(int row = ty; row < shared_width; row += TILE_WIDTH)
            for(int col = tx; col < shared_width; col += TILE_WIDTH)
                if((h_base + row) < Height && (w_base + col) < Width)
                    tile_3d(c, row, col) = in_4d(batch, c, h_base + row, w_base + col);
    __syncthreads();
    ...
    if(h < Height_out && w < Width_out){
        ...
        for(int c = 0; c < Channel; c++){
            ...
            for(int p = 0; p < K; p++){
                ...
                #pragma unroll 7
                for(int q = 0; q < K; q++)
                    acc += tile_3d(c, ty + p, tx + q) * mask_4d(map, c, p, q);
            }
        }
...
__global__ void mat_mul_conv(float* output, const float* __restrict__ input, const float* __restrict__ mask, const int Map_out, const int Channel, const int Height, const int Width, const int K) {
    ...
    //ioashdgioplhajnsdfgkl;,hjad;klsfghj
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    if(isFirstLayer)
        conv_forward_kernel<<<grid, block, sizeof(float) * Channel * (TILE_WIDTH + K - 1) * (TILE_WIDTH + K - 1)>>>(device_output, device_input, device_mask, Batch, Map_out, Channel, Height, Width, K);
    else
